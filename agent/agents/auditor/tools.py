...
def explore_directory_extended(path: str, max_depth: int = 1) -> str:
    ...
    try:
        # get extended attributes and flags
        result = subprocess.run(
            [
                "find",
                str(p),
        ...
        if result.returncode != 0:
            return json.dumps(
                {"error": f"Extended exploration failed: {result.stderr}"}
            )
...
def check_acls_and_xattrs(path: str) -> str:
    ...
    try:
        ...
        acl_result = subprocess.run(
            ["ls", "-le", str(p)], capture_output=True, text=True, timeout=10
        )
        ...
        xattr_result = subprocess.run(
            ["xattr", "-l", str(p)], capture_output=True, text=True, timeout=10
        )
        ...
        if p.is_file() and os.access(p, os.X_OK):
            codesign_result = subprocess.run(
                ["codesign", "-dv", str(p)], capture_output=True, text=True, timeout=10
            )
...
def analyze_path_security(path: str) -> str:
    ...
    try:
            ...
            try:
                ...
                if stat_result.returncode == 0:
                    ...
                    analysis["ownership_chain"].append(
                        {"path": name, "uid": uid, "gid": gid, "permissions": perms}
                    )
        ...
        if (
            analysis["permission_analysis"].get("uid")
            and int(analysis["permission_analysis"]["uid"]) != current_uid
            and int(analysis["permission_analysis"]["uid"]) != 0
        ):
...
def explore_specific_path(path: str, context_note: str = "") -> str:
    ...
    try:
        ...
        analysis = {
            "requested_path": path,
            "resolved_path": str(p),
            "relative_to_cwd": str(p.relative_to(current_dir))
            if current_dir in p.parents or p == current_dir
            else "outside_cwd",
        ...
        if p.is_dir():
            try:
                ...
                analysis["details"]["sample_children"] = [
                    child.name for child in children[:10]
                ]
                ...
                if subdirs:
                    ...
                    analysis["details"]["sample_subdirs"] = [
                        d.name for d in subdirs[:5]
                    ]
        ...
        if analysis["accessible"]:
            xattr_result = subprocess.run(
                ["xattr", "-l", str(p)], capture_output=True, text=True, timeout=5
            )

            if xattr_result.returncode == 0 and xattr_result.stdout.strip():
                analysis["details"]["has_extended_attributes"] = True
                analysis["details"]["xattr_summary"] = (
                    xattr_result.stdout.strip().split("\n")[:3]
                )  # First few lines
            else:
