...
def _get_common_tools() -> str:
    ...
    common_tool_lines.append(
        "- help_flag: Output from the `<command> --help` if available."
    )
    ...
    if tool_availability["brew_info"]:
        common_tool_lines.append(
            "- brew_info: Full `brew info` output for a Homebrew package."
        )
...
def load_prompts():
    ...
    if not prompts_path.exists():
        raise FileNotFoundError(
            f"Prompts configuration file not found at {prompts_path}. Please run 'og init'."
        )
...
def prepare_planning_prompt(query: str) -> str:
    ...
    return template.format(
        planning_tools_section_str=planning_tools_section_str, query=query
    )
...
def prepare_recipe_continuation_query(session: AgentSession) -> str:
    ...
    original_request_line = (
        session.original_query.strip() if session.original_query else "N/A"
    )
...
def prepare_fallback_continuation_query(session: AgentSession) -> str:
    ...
    return template.format(
        ...
        original_request_line=session.original_query.strip()
